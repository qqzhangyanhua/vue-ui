...
const inputs = getPackagesSync().map(pck => pck.name).filter(name => name.includes('@axzo-ui'));
export default inputs.map(name => {
    const pckName = name.split('@axzo-ui')[1] // button icon
    return {
        ...
        external(id) { // 对vue本身 和 自己写的包 都排除掉不打包
            return /^vue/.test(id) || /^@axzo-ui/.test(id)
        },
    }
})